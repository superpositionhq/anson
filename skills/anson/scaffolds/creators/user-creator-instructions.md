...
## anson-v2 note — KEY_PEOPLE.md is separate

In anson-v2 the master contacts list lives in its own `KEY_PEOPLE.md`, written after the maker loop. The user-maker may still surface relationships that matter to how the user works, but it does not need to produce an exhaustive contact roster — that is KEY_PEOPLE.md's job.
